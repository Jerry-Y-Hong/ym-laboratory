(function () {
  const STORAGE_KEY = 'ym_design_mock_state_v1';
  ...
  function loadState() {
    try {
      ...
      return {
        ...
        tabState: parsed.tabState || {},
        role: parsed.role || '',
        rdStep: Number(parsed.rdStep || 4)
      };
    } catch (e) {
      return { cart: {}, feedbacks: [], joined: {}, tabState: {}, role: '', rdStep: 4 };
    }
  }
  ...
  function money(value) {
    const locale = currentLanguage === 'en' ? 'en-US' : currentLanguage === 'ja' ? 'ja-JP' : currentLanguage === 'ar' ? 'ar-EG' : 'ko-KR';
    return '₩' + Number(value || 0).toLocaleString(locale);
  }

  function toast(message) {
    let wrap = document.getElementById('ym-mock-toast-wrap');
    if (!wrap) {
      wrap = document.createElement('div');
      wrap.id = 'ym-mock-toast-wrap';
      wrap.style.cssText = 'position:fixed;right:16px;bottom:16px;z-index:9999;display:flex;flex-direction:column;gap:8px;max-width:min(92vw,360px)';
      document.body.appendChild(wrap);
    }
    const el = document.createElement('div');
    el.textContent = message;
    el.style.cssText = 'background:rgba(9,16,28,0.94);color:#fff;border:1px solid rgba(73,205,166,0.45);border-radius:12px;padding:12px 14px;box-shadow:0 12px 32px rgba(0,0,0,0.35);font-size:13px;line-height:1.45;backdrop-filter:blur(8px);';
    wrap.appendChild(el);
    setTimeout(() => {
      el.style.opacity = '0';
  ...
  function updateCartUI() {
    ...
    const countEl = document.getElementById('mock-cart-count');
    const totalEl = document.getElementById('mock-cart-total');
    ...
    document.querySelectorAll('[data-checkout-btn]').forEach(btn => {
      btn.disabled = totals.count <= 0;
      btn.style.opacity = totals.count <= 0 ? '0.55' : '1';
      btn.style.cursor = totals.count <= 0 ? 'not-allowed' : 'pointer';
    });
  }

  function addToCart(name, price) {
    const state = loadState();
    if (!state.cart[name]) state.cart[name] = { name, price: Number(price || 0), qty: 0 };
    state.cart[name].qty += 1;
    saveState(state);
    updateCartUI();
    toast(name + ' ' + t('항목이 장바구니에 담겼습니다.'));
  }
  ...
  function bindAddToCart() {
    document.querySelectorAll('[data-add-cart]').forEach(el => {
      el.addEventListener('click', function (evt) {
        evt.preventDefault();
        const name = this.getAttribute('data-add-cart');
        const price = Number(this.getAttribute('data-price') || 0);
        const goAfterAdd = this.getAttribute('data-go-after-add');
        addToCart(name, price);
        if (goAfterAdd) {
          setTimeout(() => { window.location.href = goAfterAdd; }, 220);
        }
      });
    });
  }
  ...
  function submitFeedback(button) {
    const area = button.closest('.section')?.querySelector('textarea');
    const text = area ? area.value.trim() : '';
    if (!text) {
      toast(t('후기 내용을 입력해 주세요.'));
      return;
    }
    const state = loadState();
    state.feedbacks.unshift({ text, createdAt: new Date().toISOString() });
    saveState(state);
    if (area) area.value = '';
    button.textContent = t('등록 완료 ✓');
  ...
  async function getActiveRole() {
    if (isLocalMockHost()) return getRole();
    const remoteRole = await readRemoteSession();
    setRole(remoteRole || '');
    return remoteRole || '';
  }

  async function loginRole(role, code) {
    if (isLocalMockHost()) {
      setRole(role);
      return { ok: true, role };
    }
    try {
      ...
      return { ok: false, message: data && data.message ? data.message : t('접근 코드를 입력해 주세요.') };
    } catch (e) {
      return { ok: false, message: 'Authentication server unavailable.' };
    }
  }
  ...
  function enterRole(role, target) {
    if (role === 'consumer') {
      setRole('consumer');
  ...
  async function resetRole(target) {
    await logoutRole();
    if (target) window.location.href = target;
  }
  ...
  function getRequiredRole() {
    ...
    const expertPages = ['09-standard-7axis.html', '10-normalize-exceptions.html', '11-mapper-viewer.html', '12-rd-builder.html', '13-pro-community.html', '15-rd-business.html'];
    const adminPages = ['14-crm-admin.html'];
    if (adminPages.includes(current)) return 'admin';
    if (expertPages.includes(current)) return 'expert';
    return '';
  }

  function removeAccessGate() {
    document.body.classList.remove('ym-gated');
    const gate = document.getElementById('ym-access-gate');
    if (gate) gate.remove();
  }

  function renderAccessGate(requiredRole) {
    removeAccessGate();
    const localMode = isLocalMockHost();
    const currentRole = getRole();
    const title = requiredRole === 'admin' ? t('운영자 전용 화면입니다.') : t('전문가 전용 화면입니다.');
    const desc = requiredRole === 'admin'
      ? t('이 화면은 구독, 이탈위험, VIP, 캠페인을 관리하는 운영자 전용 흐름입니다.')
      : t('이 화면은 일반 사용자 구매 흐름이 아니라 기준·검증·협업을 다루는 전문가 전용 흐름입니다.');
    const roleLabel = currentRole === 'consumer' ? t('일반 사용자') : currentRole === 'expert' ? t('전문가') : currentRole === 'admin' ? t('운영자') : t('인증되지 않음');
    const roleHint = `${t('현재 모드')}: ${roleLabel}`;

    const style = document.createElement('style');
    style.id = 'ym-access-gate-style';
    style.textContent = `
      body.ym-gated { overflow: hidden; }
      #ym-access-gate{position:fixed; inset:0; z-index:10000; background:rgba(4,10,20,0.88); backdrop-filter:blur(12px); display:flex; align-items:center; justify-content:center; padding:24px;}
      #ym-access-gate .gate-card{width:min(92vw,720px); border:1px solid rgba(255,255,255,0.12); border-radius:22px; background:linear-gradient(180deg, rgba(11,20,35,0.96), rgba(8,14,26,0.98)); box-shadow:0 24px 64px rgba(0,0,0,0.45); padding:28px; color:#fff;}
      #ym-access-gate .gate-title{font-size:28px; font-weight:800; margin:6px 0 10px;}
      #ym-access-gate .gate-desc{font-size:15px; line-height:1.65; color:#c8cfdd; margin-bottom:14px;}
      #ym-access-gate .gate-row{display:flex; gap:10px; flex-wrap:wrap; margin-top:14px;}
      #ym-access-gate .gate-btn{border:0; border-radius:999px; padding:12px 18px; font-size:14px; font-weight:700; cursor:pointer;}
      #ym-access-gate .gate-btn.primary{background:linear-gradient(135deg,#3b84f6,#49cda6); color:#fff;}
      #ym-access-gate .gate-btn.secondary{background:rgba(255,255,255,0.08); color:#fff; border:1px solid rgba(255,255,255,0.12);}
      #ym-access-gate .gate-badge{display:inline-flex; align-items:center; gap:8px; border-radius:999px; padding:7px 12px; background:rgba(73,205,166,0.12); color:#9df0d4; font-size:12px; font-weight:700;}
      #ym-access-gate .gate-input{width:min(100%, 320px); border-radius:12px; border:1px solid rgba(255,255,255,0.16); background:rgba(255,255,255,0.06); color:#fff; padding:14px 16px; font-size:14px; outline:none;}
      #ym-access-gate .gate-help{font-size:12px; color:#9ca9c1; margin-top:10px;}
      #ym-access-gate .gate-error{font-size:13px; color:#ffb4b4; margin-top:10px; min-height:18px;}
    `;
    if (!document.getElementById('ym-access-gate-style')) document.head.appendChild(style);

    const authUi = localMode
      ? `${requiredRole === 'expert' ? `<button type=\"button\" class=\"gate-btn primary\" data-gate-enter-role=\"expert\">${t('로컬 전문가 진입')}</button>` : ''}${requiredRole === 'admin' ? `<button type=\"button\" class=\"gate-btn primary\" data-gate-enter-role=\"admin\">${t('로컬 운영자 진입')}</button>` : ''}`
      : `<div style=\"margin-top:10px\"><input class=\"gate-input\" data-gate-code type=\"password\" placeholder=\"${requiredRole === 'admin' ? t('운영자 접근 코드') : t('전문가 접근 코드')}\" /></div><div class=\"gate-row\"><button type=\"button\" class=\"gate-btn primary\" data-gate-submit>${t('코드 인증 후 들어가기')}</button></div><div class=\"gate-help\">${t('배포 환경에서는 서버 세션과 접근 코드를 통해 진입합니다.')}</div><div class=\"gate-error\" data-gate-error></div>`;

    const gate = document.createElement('div');
    gate.id = 'ym-access-gate';
    gate.setAttribute('data-access-gate', requiredRole);
    gate.innerHTML = `
      <div class="gate-card">
        <div class="gate-badge">${requiredRole === 'admin' ? t('운영자') : t('전문가')} · Access Gate</div>
        <div class="gate-title">${title}</div>
        <div class="gate-desc">${desc}</div>
        <div class="gate-desc" style="font-size:13px; color:#9ca9c1; margin-bottom:0">${roleHint}</div>
        <div class="gate-row">${authUi}</div>
        <div class="gate-row">
          <button type="button" class="gate-btn secondary" data-gate-go-consumer>${t('일반 사용자 화면으로 이동')}</button>
          <button type="button" class="gate-btn secondary" data-gate-go-home>${t('메인으로 이동')}</button>
        </div>
      </div>
    `;
    document.body.classList.add('ym-gated');
    document.body.appendChild(gate);

    gate.querySelector('[data-gate-go-consumer]')?.addEventListener('click', async () => { await resetRole('02-dashboard.html'); });
    gate.querySelector('[data-gate-go-home]')?.addEventListener('click', async () => { await resetRole('01-landing.html'); });
    gate.querySelector('[data-gate-enter-role="expert"]')?.addEventListener('click', () => enterRole('expert', pageName()));
    gate.querySelector('[data-gate-enter-role="admin"]')?.addEventListener('click', () => enterRole('admin', pageName()));
    gate.querySelector('[data-gate-submit]')?.addEventListener('click', async () => {
      const code = gate.querySelector('[data-gate-code]')?.value?.trim() || '';
      const errorEl = gate.querySelector('[data-gate-error]');
      if (!code) {
        if (errorEl) errorEl.textContent = t('접근 코드를 입력해 주세요.');
        return;
      }
      const result = await loginRole(requiredRole, code);
      if (!result.ok) {
        if (errorEl) errorEl.textContent = result.message || t('인증에 실패했습니다.');
        return;
      }
  ...
  async function ensureRoleAccess() {
    ...
    const allowed = requiredRole === 'expert' ? (role === 'expert' || role === 'admin') : role === 'admin';
    if (!allowed) {
      renderAccessGate(requiredRole);
      return false;
    }
    removeAccessGate();
    return true;
  ...
  function applySimpleTabs(tabs, activeIndex) {
  ...
  function bindSimpleTabs() {
    const page = document.querySelector('h1');
    if (!page || /(공동연구 설계기|R&D 설계기)/.test(page.textContent)) return;
    ...
    groups.forEach((group, groupIdx) => {
      ...
      const key = `${pageName()}::${groupIdx}`;
      const initial = Number.isInteger(tabState[key]) ? tabState[key] : Math.max(0, tabs.findIndex(tab => tab.classList.contains('active')));
      const apply = (idx) => {
        applySimpleTabs(tabs, idx);
        const fresh = loadState();
        fresh.tabState = fresh.tabState || {};
        fresh.tabState[key] = idx;
        saveState(fresh);
      };
      tabs.forEach((tab, idx) => {
        tab.setAttribute('role', 'button');
        tab.setAttribute('tabindex', '0');
        tab.addEventListener('click', () => apply(idx));
  ...
  function renderRdStep() {
    const page = document.querySelector('h1');
    if (!page || !/(공동연구 설계기|R&D 설계기)/.test(page.textContent)) return;

    const state = loadState();
    const step = Math.max(1, Math.min(5, Number(state.rdStep || 4)));
    const tabLabels = [t('1) 초안'), t('2) 검증'), t('3) 임상'), t('4) 등록')];
    const tabs = Array.from(document.querySelectorAll('.tabs .tab'));
    tabs.forEach((tab, idx) => {
      tab.classList.toggle('active', idx === Math.min(step - 1, 3));
      tab.textContent = tabLabels[idx];
      tab.style.cursor = 'pointer';
      tab.setAttribute('role', 'button');
      tab.setAttribute('tabindex', '0');
    });

    const statusNodes = document.querySelectorAll('.row-list .row span:last-child');
    const labels = ['✓', '✓', '✓', step >= 5 ? '✓' : '⚙ 진행', step >= 5 ? '✓ 등록 완료' : '⏸ 대기'];
    const colors = ['#49cda6', '#49cda6', '#49cda6', step >= 5 ? '#49cda6' : '#3b84f6', step >= 5 ? '#49cda6' : '#6b7390'];
    statusNodes.forEach((node, idx) => {
      if (!labels[idx]) return;
      node.textContent = labels[idx];
      node.style.color = colors[idx];
    });

    const button = document.querySelector('[data-rd-next]');
    if (button) {
      button.textContent = step >= 5 ? t('표준 DB 등록 완료 ✓') : t('다음 단계 →');
      button.disabled = step >= 5;
      button.style.opacity = step >= 5 ? '0.75' : '1';
      button.style.cursor = step >= 5 ? 'default' : 'pointer';
    }
  }

  function setRdStep(step) {
    const state = loadState();
    state.rdStep = Math.max(1, Math.min(5, Number(step || 1)));
    saveState(state);
    renderRdStep();
  }
  ...
  function bindRdTabs() {
    ...
    tabs.forEach((tab, idx) => {
      const step = idx + 1;
      tab.addEventListener('click', () => setRdStep(step));
      tab.addEventListener('keydown', (evt) => {
        if (evt.key === 'Enter' || evt.key === ' ') {
          evt.preventDefault();
          setRdStep(step);
  ...
  function advanceRdStep() {
    const state = loadState();
    state.rdStep = Math.min(5, Number(state.rdStep || 4) + 1);
    saveState(state);
    renderRdStep();
    toast(state.rdStep >= 5 ? t('R&D 등록 단계까지 진행되었습니다.') : t('다음 단계로 이동했습니다.'));
  }

  function renderCommunity() {
    const cards = document.querySelectorAll('[data-join-id]');
    if (!cards.length) return;
    const state = loadState();
    cards.forEach(link => {
      const id = link.getAttribute('data-join-id');
      const joined = !!state.joined[id];
      link.textContent = joined ? t('참가 완료 ✓') : t('참가 →');
      link.style.background = joined ? '#49cda6' : '';
    });
  }

  function joinProgram(link) {
    const id = link.getAttribute('data-join-id');
    const state = loadState();
    const meta = link.parentElement?.querySelector('.meta');
    if (!state.joined[id]) {
      ...
      toast(t('프로그램 참가가 완료되었습니다.'));
    } else {
      toast(t('이미 참가한 프로그램입니다.'));
    }
    saveState(state);
    renderCommunity();
  }
  ...
  function checkout() {
    ...
    const totals = cartTotals(state);
    if (totals.count <= 0) {
      toast(t('장바구니가 비어 있습니다.'));
      return;
    }
    state.cart = {};
    saveState(state);
    updateCartUI();
    toast(t('모의 결제가 완료되었습니다.'));
  }
