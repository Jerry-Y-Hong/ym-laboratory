...
for idx, row in df_recipes_raw.iterrows():
    ...
    comp_type = "SUB_SIDE (보조 부식)"
    if any(k in food_nm for k in ["밥", "죽", "면", "만두", "스낵", "쿠키", "케이크", "덮밥", "식해"]):
        comp_type = "STAPLE (주식)"
    elif any(k in food_nm for k in ["탕", "국", "찌개", "전골", "매운탕"]):
        comp_type = "SOUP_STEW (탕류)"
...
kit_data = [
    {
        "KIT_ID": "K01",
        "KIT_NAME": "기력보강 앰플키트 (Energy Recovery Ampoule Kit)",
        ...
        "DOSAGE_FORMAT": "10ml Liquid Ampoule / 15g Tea bag filter",
        "FRANCHISE_KITCHEN_GUIDE": "조리 단계 후반 5분 전에 키트(티백/앰플)를 투입하여 유효성분 우려냄.",
